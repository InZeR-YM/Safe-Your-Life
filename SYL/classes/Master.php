...
Class Master extends DBConnection {
	...
	function save_category(){
		...
		if($save){
			...
			$resp['status'] = 'success';
			if(empty($id))
				$resp['msg'] = "Новая категория успешно создана.";
			else
				$resp['msg'] = " Категория успешно изменена.";
		}else{
			$resp['status'] = 'failed';
	...
	function save_product(){
		...
		if($save){
			...
			$resp['status'] = 'success';
			if(empty($id)){
				$resp['msg'] = 'Товар успешно добавлен.';
			}else{
				$resp['msg'] = " Товар успешно изменён.";
			}
	...
	function delete_product(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Товар успешно удалён.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_stock(){
		...
		if($save){
			...
			$resp['status'] = 'success';
			if(empty($id))
				$resp['msg'] = "Новая позиция успешно добавлена.";
			else
				$resp['msg'] = " Позиция успешно изменена.";
		}else{
			$resp['status'] = 'failed';
			$resp['err'] = $this->conn->error."[{$sql}]";
		}
	...
	function delete_stock(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Позиция успешно удалена.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function add_to_card(){
		...
		if($resp['status'] == 'success'){
			$this->settings->set_flashdata('success', 'Товар добавлен в корзину.');
		}
		return json_encode($resp);
	}
	function update_cart(){
		...
		if($resp['status'] == 'success'){
			$this->settings->set_flashdata('success', 'Товар в корзине изменён.');
		}
		return json_encode($resp);
	}
	function delete_cart(){
		...
		if($resp['status'] == 'success'){
			$this->settings->set_flashdata('success', 'Товар удалён из корзины.');
		}
		return json_encode($resp);
	}
	function place_order(){
		...
		if($resp['status'] == 'success'){
			$this->settings->set_flashdata('success', 'Заказ успешно создан.');
		}
		return json_encode($resp);
	}
	function update_order_status(){
		...
		if($resp['status'] == 'success')
		$this->settings->set_flashdata('success', "Статус заказа успешно изменён.");
	...
	function delete_order(){
		...
		if($resp['status'] == 'success'){
			$this->settings->set_flashdata('success', 'Заказ успешно удалён.');
		}
		return json_encode($resp);
	}
	function save_inquiry(){
		...
		if($save){
			...
			$resp['status'] = 'success';
			if(empty($id))
				$this->settings->set_flashdata('success'," Ваш запрос был успешно отправлен. Спасибо!");
			else
				$this->settings->set_flashdata('success'," Запрос успешно изменён.");
	...
	function delete_inquiry(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Запрос успешно удалён.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
