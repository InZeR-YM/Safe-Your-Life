...
Class Users extends DBConnection {
	...
	public function save_users(){
		...
		if(empty($id)){
			...
			if($qry){
				...
				$this->settings->set_flashdata('success','Информация пользователя успешно сохранена.');
				foreach($_POST as $k => $v){
					if($k != 'id'){
						if(!empty($data)) $data .=" , ";
			...
			if($qry){
				$this->settings->set_flashdata('success','Информация пользователя успешно изменена.');
				foreach($_POST as $k => $v){
					if($k != 'id'){
						if(!empty($data)) $data .=" , ";
						if($this->settings->userdata('id') == $id)
	...
	public function delete_users(){
		...
		if($qry){
			$this->settings->set_flashdata('success','Информация пользователя успешно удалена.');
	...
	function registration(){
		...
		if($save){
			...
			$resp['uid'] = $uid;
			if(!empty($id))
				$resp['msg'] = 'Информация пользователя успешно изменена';
			else
				$resp['msg'] = 'Ваш аккаунт успешно создан';
			...
			if(!empty($_FILES['img']['tmp_name'])){
				...
				$accept = array('image/jpeg','image/png');
				if(!in_array($_FILES['img']['type'],$accept)){
					$resp['msg'] = "Неверный формат файла";
				}
				if($_FILES['img']['type'] == 'image/jpeg')
					$uploadfile = imagecreatefromjpeg($_FILES['img']['tmp_name']);
				elseif($_FILES['img']['type'] == 'image/png')
	...
	public function delete_customer(){
		...
		if($qry){
			$this->settings->set_flashdata('success','Информация о пользователе успешно удалена.');
			$resp['status'] = 'success';
