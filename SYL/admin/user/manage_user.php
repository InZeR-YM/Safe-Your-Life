...
<div class="card card-outline rounded-0 card-danger">
	<div class="card-body">
		<div class="container-fluid">
			...
			<form action="" id="manage-user">	
				...
				<div class="form-group">
					<label for="password"><?= isset($meta['id']) ? "Новый" : "" ?> пароль</label>
					<input type="password" name="password" id="password" class="form-control" value="" autocomplete="off">
                    <?php if(isset($meta['id'])): ?>
					<small><i>Оставьте это поле пустым, если не хотите изменять пароль.</i></small>
                    <?php endif; ?>
				</div>
                <div class="form-group">
                    ...
                    <option value="2" <?php echo isset($meta['type']) && $meta['type'] == 2 ? 'selected' : '' ?>>Сотрудник</option>
                    </select>
                </div>
				<div class="form-group">
	...
	<div class="card-footer">
			<div class="col-md-12">
				<div class="row">
					<button class="btn btn-sm btn-primary rounded-0 mr-3" form="manage-user">Сохранить данные пользователя</button>
					<a href="./?page=user/list" class="btn btn-sm btn-default border rounded-0" form="manage-user"><i class="fa fa-angle-left"></i> Отмена</a>
				</div>
			</div>
		</div>
</div>
		...
		$.ajax({
			...
			success:function(resp){
				if(resp ==1){
					...
					$('#msg').html('<div class="alert alert-danger">Такой пользователь уже существует</div>')
					end_loader()
				}
			}
		})
