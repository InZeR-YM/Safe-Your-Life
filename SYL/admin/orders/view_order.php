...
<div class="content py-5 px-3 bg-gradient-danger">
	<h2><b>Заказ №<?= isset($code) ? $code : '' ?></b></h2>
</div>
            ...
            <div class="card-header py-1">
                <div class="card-tools">
                    <?php if(isset($status) && $status < 4): ?>
                        <button class="btn btn-info btn-sm bg-gradient-info rounded-0" type="button" id="update_status">Изменить статус</button>
                    <?php endif; ?>
                    <button class="btn btn-navy btn-sm bg-gradient-navy rounded-0" type="button" id="print"><i class="fa fa-print"></i> Печать</button>
                    ...
                    <a class="btn btn-light btn-sm bg-gradient-light border rounded-0" href="./?page=orders"><i class="fa fa-angle-left"></i> Вернуться к списку</a>
                </div>
            </div>
        </div>
    </div>
                ...
                <div class="container-fluid">
                    <div class="row mb-3">
                        <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                            <div class="mb-3">
                                <label for="" class="control-label">Номер заказа:</label>
                                <div class="pl-4"><?= isset($code) ? $code : '' ?></div>
                            </div>
                            <div class="mb-3">
                                <label for="" class="control-label">Адрес доставки:</label>
                        ...
                        <div class="mb-3">
                                ...
                                $status = isset($status) ? $status : '';
                                    switch($status){
                                        case 0:
                                            echo '<span class="badge badge-secondary bg-gradient-secondary px-3 rounded-pill">Выполняется</span>';
                                            break;
                                        case 1:
                                        ...
                                        case 3:
                                            ...
                                            break;
                                        default:
                                            echo '<span class="badge badge-light bg-gradient-light border dark px-3 rounded-pill">Неизвестно</span>';
                                            break;
                                    }
                                ?>
                                </div>
                                ...
                                <div class="col-auto flex-shrink-1 flex-grow-1">
                                    <div style="line-heigth:1em">
                                        ...
                                        <div class="text-muted d-flex w-100">
                                            <?= format_num($row['quantity'],0) ?> шт. x <?= format_num($row['price'],2) ?> BYN
                                        </div>
                                    </div>
                                </div>
                                <div class="col-auto">
                                    <h4><b><?= format_num($row['price'] * $row['quantity'], 2) ?> BYN</b></h4>
                                </div>
                            </div>
                        </div>
                        <?php endwhile; ?>
                    ...
                    <?php if($order_items->num_rows <= 0): ?>
                        <h5 class="text-center text-muted">Заказ пуст</h5>
                    <?php endif; ?>
                    <div class="d-flex justify-content-end py-3">
                        <div class="col-auto">
                            <h3><b>Итого: <?= format_num($gt,2) ?> BYN</b></h3>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="d-flex w-100 align-items-center">
            ...
            <div class="col-8">
                <div style="line-height:1em">
                    ...
                    <div class="text-center font-weight-bold"><large>Детали заказа</large></div>
                </div>
            </div>
        </div>
...
<script>
    function print_t(){
        ...
        h.find('title').text("Детали заказа - Просмотр печати")
    ...
    $(function(){
        ...
        $('#assign_team').click(function(){
            uni_modal("Передать команде", 'orders/assign_team.php?id=<?= isset($id) ? $id : '' ?>')
        })
		$('#delete_data').click(function(){
			_conf("Вы уверены, что хотите безвозвратно удалить этот заказ?","delete_order", ["<?= isset($id) ? $id :'' ?>"])
		})
        $('#update_status').click(function(){
            uni_modal("Изменить статус", "orders/update_status.php?id=<?= isset($id) ? $id : '' ?>")
        })
    })
