...
<div class="card card-outline rounded-0 card-danger">
	<div class="card-header">
		...
		<div class="card-tools">
			<a href="./?page=products/manage_product" id="create_new" class="btn btn-flat btn-primary"><span class="fas fa-plus"></span>  Создать</a>
		</div>
	</div>
	<div class="card-body">
				...
				<tbody>
					...
					?>
						<tr>
							...
							<td class="text-right"><?= format_num($row['price'],2) ?> BYN</td>
							<td class="text-center">
                                <?php if($row['status'] == 1): ?>
							...
							<td align="center">
								 <button type="button" class="btn btn-flat p-1 btn-default btn-sm dropdown-toggle dropdown-icon" data-toggle="dropdown">
				                  		Действие
				                    <span class="sr-only">Показать выпадающий список</span>
				                  </button>
				                  <div class="dropdown-menu" role="menu">
				                    <a class="dropdown-item" href="./?page=products/view_product&id=<?php echo $row['id'] ?>"><span class="fa fa-eye text-dark"></span> Просмотр</a>
				                    <div class="dropdown-divider"></div>
				                    <a class="dropdown-item" href="./?page=products/manage_product&id=<?php echo $row['id'] ?>"><span class="fa fa-edit text-primary"></span> Изменить</a>
				                    <div class="dropdown-divider"></div>
...
<script>
	$(document).ready(function(){
		$('.delete_data').click(function(){
			_conf("Вы уверены, что хотите удалить этот товар безвозвратно?","delete_product",[$(this).attr('data-id')])
		})
		$('.table').dataTable({
			columnDefs: [
